...
import {apiRoutes, decodeDataFromBase64} from "@/config/common";
...
const MessageBody = () => {
    ...
    const chatId = useMemo(() => decodeDataFromBase64(searchParams.get("chat") || "")?.id || 0, [searchParams]);
    const projectId = useMemo(() => decodeDataFromBase64(searchParams.get("project") || "")?.id, [searchParams]);
...
const MessageBodyChat = ({messages}: { messages: MessageBodyChatItemInterface[] }) => {
...
const MessageBodyChatItem = ({data}: { data: MessageBodyChatItemInterface }) => {
