...
const Page = ({
  ...
  const { data } = useDataFetch<IEvent>(
    apiRoutes.PROTECTED.GENERAL.MEETINGS.GET(params?.slug)({
      expand:
        "slot,client,client.user_details,client.user_details.profile_picture,project",
    }),
  );
  return (
    <div className="p-4">
      ...
      <BreadcrumbList>
        <BreadcrumbItem>
          <BreadcrumbLink
            ...
            href={frontendLinks.PRIVATE.CALENDER}
          >
            Calender
          </BreadcrumbLink>
        </BreadcrumbItem>
...
const LeftSection = ({ data: clientData }: { data?: IClient }) => {
  ...
  const { data: chats } = useAxiosSWR(
    apiRoutes.PROTECTED.GENERAL.CHAT.LIST({
      limit: 1,
      expand: "receiver",
  ...
  async function onClick() {
    if (chats?.length) {
      return navigate.push(
        frontendLinks.PRIVATE.MESSAGE(encodeDataToBase64(chats?.[0])),
      );
    }
    ...
    return navigate.push(
      frontendLinks.PRIVATE.MESSAGE(encodeDataToBase64(chat)),
    );
  }
    ...
    <div className="col-span-3 flex flex-col items-center rounded-lg bg-white p-2">
      ...
      <div className="mt-6 flex w-3/4 flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <p>Member Since</p>
          <p>{clientData && timezoneToDDMMYYYY(clientData?.date_joined)}</p>
        </div>
      </div>
    </div>
  );
...
const MeetingDetails: React.FC<MeetingDetailsProps> = ({ project }) => {
  return (
    <div className="">
  ...
  return (
    <div>
      <div className="my-4 flex gap-2 font-bold">
        <p>Date and Time: </p>
        <p>
          {date} {convertTo12Hour(slot?.start_at)}
        </p>
        <p>TO</p>
        <p>
          {date} {convertTo12Hour(slot?.end_at)}
        </p>
      </div>
    </div>
  );
